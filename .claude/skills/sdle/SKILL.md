...
Then initialise with **`sdle.sh --workitem <id> init`**, which sets `project_name` (`--project` if given, else the first `#` heading in the binding's **primary** document, else the WorkItem's title, else the project root's directory name), writes the first audit entries — including `flow_selected`, which records which lifecycle was bound and why — and advances to the bound flow's first generation phase (`constitution_draft` under GREENFIELD, `discovery` under `BROWNFIELD_DISCOVERY`, `impact_analysis` under the two defect flows). Take the phase from the response, never from a table. Name the id you just created: resolution exists for later turns, and at bootstrap you already know the answer. The WorkItem is the durable identity **and** the runtime scope: `init` writes `workitems/<id>/.sdle/state.json`, `state.json` records which WorkItem it belongs to, and `execution.json` records the branch and starting SHA this run began on. If a `.workflow/state.json` from a retired runtime is present, `init` refuses `legacy_workflow_present`: SDLE does not run or migrate it and leaves it untouched, so remove it or move it aside first.

**Resolution on later turns.** Every runtime command resolves a WorkItem first, highest priority first: an explicit `--workitem <id>`; else the directory Claude was launched from, when it is inside `workitems/<x>/`; else the sole registered WorkItem; else a still-valid persisted active context (**`sdle.sh workitem use --workitem <id>`** sets it for this working directory, `--clear` removes it); else a unique Git-branch match. There is no sixth rung: a repository-global `.workflow/state.json` from a retired runtime never binds. With none registered the command refuses `workitem_required`, and when that file is on disk the refusal says SDLE does not run or migrate it and points at `workitem create`; standing inside an unregistered `workitems/<x>/` it refuses `workitem_unregistered` rather than binding a neighbour; with several plausible and none named it refuses `workitem_ambiguous` and lists the candidates.

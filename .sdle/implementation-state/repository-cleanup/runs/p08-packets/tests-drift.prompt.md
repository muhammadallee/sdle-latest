...
## Your area: Tests and drift checks
Paths: `tests requirements-dev.txt`
Tests deleted or weakened without equivalent protection (compare what the deleted history-pinned tests protected with the replacement in tests/test_units_invariants.py); checks that pass vacuously; negative fixtures that would not actually fail; helper code left dead.

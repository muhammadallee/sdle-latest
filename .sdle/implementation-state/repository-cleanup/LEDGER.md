...
| T-CX-005 | low | **ACCEPTED**, fixed `86abcec`, **and repaired at P09** | `legacy_workflow`, its imports and the stale migration section removed from `test_units_workitem_runtime.py`. The reviewer was right that nothing *called* the helper, but `test_units_hardening.py` still *imported* it (as an unused alias), so that module stopped collecting. My targeted runs after the change did not include it and the round-1 suite result predates the change; the first full run at P09 (`20260920T181644-p09-final-full-suite`, INTERRUPTED at collection, 1 error) caught it. Unused import removed; `pytest --collect-only` now collects all 3024 tests and `test_units_hardening.py` passes (27). Lesson recorded: a deletion is checked with a whole-suite collection, not only the modules I expect to be affected |

...
**Remaining P02 tasks:** T05 F-018 (skill description; the `_check_version_consistency` frontmatter regex must tolerate an absent frontmatter version), T06 F-026 (`sdle.sh` mode), T07 F-024 (policy behavioural test), T08 (record D-01/D-04/D-05 outcomes).
...
| F-024 | — | HYPOTHESIS | not yet tested: live policy re-derivation vs pinned; behavioral test planned in P02 (D-04) | P02 | AC-01 |
...
| D-02 | Supported Python matrix (F-021) | **Default:** CI stays 3.11 on Linux + Windows and adds the newest CPython the suite passes on; document exactly those. Local evidence so far is 3.13.0 on Windows only | P06 | pending |
| D-03 | License (F-023) | **Default:** add none; report the absence as an owner action | P09 report | pending |
| D-04 | Active WorkItem and policy change (F-024) | **Default:** keep live re-derivation and document it, unless the behavioural test shows a mid-flight relaxation can drop a gate required at start — then report a confirmed defect and ask the owner before pinning or fencing (`.sdle/implementation-state/` must stay writable) | P02 | pending the test |
| D-05 | Primary beginner entry point (F-018) | **Default:** `start workflow` primary, `/sdle-start` documented as the verified equivalent | P04 | pending |
